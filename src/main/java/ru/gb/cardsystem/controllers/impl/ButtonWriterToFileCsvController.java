...
import ru.gb.cardsystem.service.impl.FileCsvWriteService;
...
public class ButtonWriterToFileCsvController extends JComponentWriterController<JButton> {
    ...
    protected ButtonWriterToFileCsvController() {
        super(new ArrayList<>(), new FileCsvWriteService());
        this.textFieldController = new TextFieldController();
        init();
    }

    private void init() {
        final JButton writeInLogger = new JButton("Write in File.csv");
